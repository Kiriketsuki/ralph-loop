...
- **Council and Verification Are Mandatory**: Never set `MISSION_COMPLETE` directly from a regular task iteration. Always go through `COUNCIL_PENDING` (adversarial review) then `VERIFICATION_PENDING` (acceptance criteria check) first.
